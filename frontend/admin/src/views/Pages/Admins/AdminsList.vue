  ...
  <admin-layout>
    <div>
      ...
      >
        ...
        <template #name="{data}">
          <router-link :to="{name: 'admin', params: {id: data.id}}" class="flex items-center gap-3">
            <div class="w-10 h-10 overflow-hidden rounded-full">
              <img :src="(data.img) ? baseUrl+data.img : `${base}images/user/owner.jpg`" :alt="data.name">
            </div>
            <div>
        ...
        <template #rule="{data}">
          <router-link :to="{name: 'admins.rules.edit', params: {id: data.rule_id}}" class="flex items-center gap-3">
            <div>
              <span class="inline-flex items-center px-2.5 py-0.5 justify-center gap-1 rounded-full font-medium capitalize text-sm bg-blue-light-500 text-white dark:text-white">{{ data.rule }}</span>
            </div>
          </router-link>
        </template>
        <template #action="{data}">
          <div class="flex items-right w-full gap-2">
            <router-link :to="{name: 'admin', params: {id: data.id}}" class="text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white/90">
